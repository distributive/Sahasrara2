...
module.exports = {
  ...
  async execute(interaction, client) {
    ...
    const color = userIsCorp ? +process.env.COLOR_CORP : +process.env.COLOR_RUNNER;

    let message;
    if (userIsCorp) {
      message = `You will corp.\n${opponent ? opponent : "Your opponent"} will run.`
    } else {
      message = `You will run.\n${opponent ? opponent : "Your opponent"} will corp.`
    }
